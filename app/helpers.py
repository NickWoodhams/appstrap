...
def firstname(full_name):
    try:
        ...
        return random.choice(gangster_nouns)
...
def transactional_email(json_obj, subject, recipient_email, recipient_name,
                        reply_to=None, bcc_admin=True):
    # new Object name
    object_dict = json_obj
    html_email = str()
    plain_email = str()

    for key, value in object_dict.items():
        if value and key != 'password' and key != 'id' and key != 'csrf_token':
            html_email += "%s: %s <br>" % (
